...
SQLite at `storage/nightshift.db` stores people, instance overrides, applied plans and forecast history. A DBstudios MariaDB workspace was created separately; the app does **not** sync with that workspace. SQLite makes the local demo usable without cloud credentials. Do not treat the separate empty cloud project as a backup.
